        ...
        <el-table :data="data.top_models" stripe style="width: 100%" size="small" max-height="400">
          <el-table-column prop="rank" label="排名" width="70" align="center">
            <template #default="{ row }">
              <el-tag :type="row.rank <= 3 ? 'danger' : row.rank <= 6 ? 'warning' : 'info'" size="small">
                #{{ row.rank }}
              </el-tag>
            </template>
          </el-table-column>
      ...
      <div v-else>
        <div class="month-selector">
          <el-select v-model="selectedMonth" placeholder="选择月份" size="small" style="width: 150px">
            <el-option 
              v-for="item in monthlyTopModelMonths" 
              :key="item" 
              :label="item" 
              :value="item" 
            />
...
const currentMonthlyTopModels = computed(() => {
  ...
  const found = props.data.monthly_top_models.find(m => m.month === selectedMonth.value)
  return found?.items || []
})
...
watch(monthlyTopModelMonths, (months) => {
  ...
  if (latest && !selectedMonth.value) {
    selectedMonth.value = latest // 默认选最新月份
  }
}, { immediate: true })
...
const getYearlyOption = (): echarts.EChartsOption => {
  ...
  return {
    ...
    yAxis: {
      type: 'value',
      name: 'DPPM',
      axisLabel: { formatter: (value: number) => value.toLocaleString(), color: '#606266' },
...
const getTimelineOption = (): echarts.EChartsOption => {
  ...
  return {
    tooltip: {
      ...
      formatter: (params: any) => {
        const data = params[0]
        return `${data.name}<br/>IFIR: ${Math.round(data.value).toLocaleString()} DPPM`
      }
    },
    grid: {
      left: '3%', right: '4%',
      bottom: trendData.length > 12 ? '60px' : '3%',
    ...
    xAxis: {
      ...
      axisLine: { lineStyle: { color: '#e4e7ed' } },
      axisLabel: { color: '#606266', rotate: trendData.length > 12 ? 45 : 0 }
    },
    yAxis: {
      type: 'value',
      name: 'DPPM',
