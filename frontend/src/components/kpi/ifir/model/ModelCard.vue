<template>
  <div class="model-card">
    ...
    <div class="card-section">
      <div class="section-header">
        <h3 class="section-title"><el-icon><WarningFilled /></el-icon>Top Issue</h3>
        <el-radio-group v-model="topIssueTab" size="small">
          <el-radio-button value="summary">汇总</el-radio-button>
          <el-radio-button value="monthly">月度</el-radio-button>
        </el-radio-group>
      </div>
      <div v-if="topIssueTab === 'summary'">
        <el-table :data="data.top_issues" stripe style="width: 100%" size="small" max-height="400" @row-click="handleIssueRowClick" empty-text="无Top Issue">
          ...
          <el-table-column label="不良现象*频次" min-width="220">
            <template #default="{ row }">
              <el-link class="issue-link" type="primary" :underline="false" @click.stop.prevent="openIssueDetails(row.issue)">
                {{ formatIssueLabel(row) }}
              </el-link>
            </template>
          </el-table-column>
      ...
      <div v-else>
        <div class="month-selector">
          <el-select v-model="selectedIssueMonth" placeholder="选择月份" size="small" style="width: 150px">
            <el-option v-for="item in monthlyIssueMonths" :key="item" :label="item" :value="item" />
          </el-select>
        </div>
        <el-table :data="currentMonthlyIssues" stripe style="width: 100%" size="small" max-height="400" @row-click="handleIssueRowClick" empty-text="无Top Issue">
          <el-table-column prop="rank" label="排名" width="70" align="center">
            <template #default="{ row }"><el-tag :type="row.rank <= 3 ? 'danger' : row.rank <= 6 ? 'warning' : 'info'" size="small">#{{ row.rank }}</el-tag></template>
          </el-table-column>
          <el-table-column label="不良现象*频次" min-width="220">
...
import { getIfirModelIssueDetails, type ModelCard as ModelCardData, type IssueDetailRow } from '@/api/ifir'
...
// Top Issue Tab切换
const topIssueTab = ref<'summary' | 'monthly'>('summary')
const selectedIssueMonth = ref('')
const monthlyIssueMonths = computed(() => (props.data.monthly_top_issues ?? [])
  .map(m => m.month)
  .filter((month): month is string => !!month))
const currentMonthlyIssues = computed(() => {
  if (!selectedIssueMonth.value || !props.data.monthly_top_issues) return []
  return props.data.monthly_top_issues.find(m => m.month === selectedIssueMonth.value)?.items || []
})
watch(monthlyIssueMonths, (months) => {
  const latest = months[months.length - 1]
  if (latest && !selectedIssueMonth.value) selectedIssueMonth.value = latest
}, { immediate: true })
...
const formatIssueLabel = (row: { issue: string; count: number }) => `${row.issue} * ${row.count}`

const openIssueDetails = async (issue: string) => {
  if (!issue) return
...
const handleIssueRowClick = (row: { issue?: string }) => {
  if (row?.issue) {
    openIssueDetails(row.issue)
  }
}
...
const getTimelineOption = (): echarts.EChartsOption => {
  ...
  return {
    ...
    series: [{
      ...
      lineStyle: { width: 3, color: '#e6a23c' }, itemStyle: { color: '#e6a23c' },
...
.model-card { background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); }
.card-header { display: flex; justify-content: space-between; align-items: center; padding: 16px 20px; background: linear-gradient(135deg, #e6a23c 0%, #c78c2e 100%); color: #fff;
...
.card-section { padding: 20px; border-bottom: 1px solid var(--border-color);
  ...
  .section-title { display: flex; align-items: center; gap: 8px; font-size: 15px; font-weight: 500; color: var(--text-color); margin: 0; .el-icon { color: #e6a23c; } }
}
...
.ai-summary {
  ...
  .summary-content { padding: 16px; background: #fdf6ec; border-radius: 8px; color: #666; line-height: 1.8; font-size: 14px; }
}
.detail-content { p { margin: 12px 0; font-size: 14px; } }
.month-selector { margin-bottom: 12px; }
.issue-link { display: block; width: 100%; }
