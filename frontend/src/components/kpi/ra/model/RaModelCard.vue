  ...
  <div class="ra-model-card">
    ...
    <div class="card-section">
      ...
      <div v-if="topIssueTab === 'summary'">
        <el-table :data="data.top_issues" stripe style="width: 100%" size="small" max-height="400" @row-click="(row: any) => row?.issue && openIssueDetails(row.issue)" empty-text="无Top Issue">
          ...
          <el-table-column label="不良现象*频次" min-width="220">
            <template #default="{ row }">
              <span class="issue-link" @click.stop="handleIssueRowClick(row)">
      ...
      <div v-else>
        ...
        <el-table :data="currentMonthlyIssues" stripe style="width: 100%" size="small" max-height="400" @row-click="(row: any) => row?.issue && openIssueDetails(row.issue)" empty-text="无Top Issue">
          <el-table-column prop="rank" label="排名" width="70" align="center">
            <template #default="{ row }"><el-tag :type="row.rank <= 3 ? 'danger' : row.rank <= 6 ? 'warning' : 'info'" size="small">#{{ row.rank }}</el-tag></template>
          </el-table-column>
          <el-table-column label="不良现象*频次" min-width="220">
...
import { getRaModelIssueDetails, type ModelCard as ModelCardData, type IssueDetailRow } from '@/api/ra'
...
const openIssueDetails = (issue: unknown) => {
  const normalizedIssue = String(issue ?? '').trim()
  if (!normalizedIssue) return
  currentIssue.value = normalizedIssue
  currentIssueMonth.value = topIssueTab.value === 'monthly' ? selectedIssueMonth.value : ''
  issuePage.value = 1
  issueDialogVisible.value = true
  void nextTick().then(fetchIssueDetails)
}

const handleIssueRowClick = (row: { issue?: unknown } | unknown) => {
  const rawIssue =
    typeof row === 'object' && row !== null && 'issue' in row
      ? (row as { issue?: unknown }).issue
      : row
  openIssueDetails(rawIssue)
}
...
const fetchIssueDetails = async () => {
  ...
  try {
    const result = await getRaModelIssueDetails({
      ...
      odms: props.odms?.length ? props.odms : undefined,
...
.ra-model-card { background: #fff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1); }
...
.issue-link { display: block; width: 100%; color: #409eff; cursor: pointer; }
